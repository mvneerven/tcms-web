        ...
        finish: function (survey) {
                ...
                if (q.tags) {
                    ...
                    for (var t in q.tags) {
                        if (q.tags[t].find(element => element == ax) > -1) {
                            survey.selectedTags[t] = (survey.selectedTags[t] == undefined) ? 1 : survey.selectedTags[t] + 1;
                        }
                    }
                }
            };
            ...
            for (var i = 0; i < survey.questions.length; i++) {
                ...
                if (q.group && (q.group != curGroup)) {
                    curGroup = q.group;
                    groups[curGroup] = groups[curGroup] || { max: 0, score: 0, questions: {} };
                    groupIndex = 0;
                }
                else {
                    groupIndex++;
            ...
            for (var g in groups) {
                ...
                if (groups[g].max > 0) {
                    ...
                    groups[g].percent = groups[g].max == 0 ? 0 : Math.round(groups[g].score / groups[g].max * 100);
            ...
            var info = $('<div class="col-lg-5 col-sm-12 scan-info"></div>').appendTo(conclusion);

            $('<h4>Company</h4><div>' + survey.namedAnswers["company"] + '</div>').appendTo(info);
            $('<h4>Stage</h4><div>' + survey.namedAnswers["stage"].split('[')[0] + '</div>').appendTo(info);
            $('<h4>Your name</h4><div>' + survey.namedAnswers["fullName"] + '</div>').appendTo(info);
            $('<h4>Your email</h4><div>' + survey.namedAnswers["email"] + '</div>').appendTo(info);
            ...
            var reliability = Math.round(100 - (survey.unanswered / survey.ratedQuestions * 100));
            ...
            if (s.length) {
                $('<div class="col-12"><h3>Labels</h3><span class="badge-pill badge-success">' + s.join('</span><span class="badge-pill badge-success">') + '</div>').appendTo(vis)
            }
            if (r.length) {
                $('<div class="col-12"><h3>Possible Risks</h3><span class="badge-pill badge-warning">' + r.join('</span><span class="badge-pill badge-warning">') + '</div>').appendTo(vis)
            }
            if (t.length) {
                $('<div class="col-12"><h3>Possible Conflicts</h3><span class="badge-pill badge-danger">' + t.join('</span><span class="badge-pill badge-danger">') + '</div>').appendTo(vis)
            }
